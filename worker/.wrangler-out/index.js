...
var handleError$1 = /* @__PURE__ */ __name(async (error3, reject, options) => {
  if (error3 instanceof await resolveResponse$1() && !(options === null || options === void 0 ? void 0 : options.noResolveJson))
    ...
    reject(new StorageUnknownError(_getErrorMessage$1(error3), error3));
}, "handleError$1");
  ...
  return new Promise((resolve, reject) => {
    fetcher(url, _getRequestParams$1(method, options, parameters, body)).then((result) => {
      ...
      if (options === null || options === void 0 ? void 0 : options.noResolveJson)
        return result;
      return result.json();
    }).then((data) => resolve(data)).catch((error3) => handleError$1(error3, reject, options));
  });
}
...
var handleError = /* @__PURE__ */ __name(async (error3, reject, options) => {
  if (error3 && typeof error3 === "object" && "status" in error3 && "ok" in error3 && typeof error3.status === "number" && !(options === null || options === void 0 ? void 0 : options.noResolveJson)) {
    ...
    reject(new StorageVectorsUnknownError(_getErrorMessage(error3), error3));
}, "handleError");
...
var PROFILE = {
  ...
  about: "I build intelligent, scalable, and user\u2011centric AI/ML solutions with an MLOps mindset\u2014clean data, reliable pipelines, measurable outcomes, and deployments that stay stable when they meet real users. Blending AI/ML, full\u2011stack development, and cloud tools, I can ship the model, the API, and the UI as one cohesive product. I\u2019m experienced in prompt engineering, Git & GitHub, motion UI, responsive design, and accessibility\u2014so the work is not only accurate, but also usable and polished. Beyond building, I stay active in leadership and event management: mentoring juniors, organizing workshops, and contributing to cultural and adventure initiatives\u2014strengthening teamwork, strategic planning, and communication. I\u2019m always looking for opportunities to apply technical depth to real\u2011world challenges and deliver practical, innovative solutions.",
  links: {
...
function pickOrigin(req, env2) {
  const origin = req.headers.get("Origin") ?? "";
  const allow = parseCorsOrigins(env2.CORS_ORIGINS);
  if (allow.includes("*"))
    return "*";
  if (!origin)
    return allow[0] ?? "*";
...
function jsonResponse(req, env2, body, status = 200) {
  const origin = pickOrigin(req, env2);
...
function errorResponse(req, env2, status, message, details) {
  return jsonResponse(req, env2, { error: message, details }, status);
}
__name(errorResponse, "errorResponse");
function requireAdminToken(req, env2) {
  const required = (env2.ADMIN_TOKEN ?? "").trim();
  if (!required)
    return null;
  const provided = (req.headers.get("X-Admin-Token") ?? "").trim();
  if (!provided || provided !== required) {
    return errorResponse(req, env2, 401, "Unauthorized");
  }
  return null;
}
...
var src_default = {
  async fetch(req, env2) {
    ...
    if (!url.pathname.startsWith("/api/")) {
      return errorResponse(req, env2, 404, "Not found");
    }
    try {
      if (req.method === "GET" && url.pathname === "/api/health") {
        return jsonResponse(req, env2, { status: "ok" }, 200);
      }
      if (req.method === "GET" && url.pathname === "/api/profile")
        return jsonResponse(req, env2, PROFILE);
      if (req.method === "GET" && url.pathname === "/api/experience")
        return jsonResponse(req, env2, EXPERIENCE);
      if (req.method === "GET" && url.pathname === "/api/education")
        return jsonResponse(req, env2, EDUCATION);
      if (req.method === "GET" && url.pathname === "/api/certifications")
        return jsonResponse(req, env2, CERTIFICATIONS);
      if (req.method === "GET" && url.pathname === "/api/activities")
        return jsonResponse(req, env2, ACTIVITIES);
      const supabase = getSupabase(env2);
      if (url.pathname === "/api/projects" && req.method === "GET") {
        const { data, error: error3 } = await supabase.from("projects").select("*").order("created_at", { ascending: false });
        if (error3)
          return errorResponse(req, env2, 500, "Failed to fetch projects", error3);
        return jsonResponse(req, env2, data ?? [], 200);
      }
      if (url.pathname === "/api/projects" && req.method === "POST") {
        const auth = requireAdminToken(req, env2);
        if (auth)
          return auth;
        const payload = await req.json().catch(() => ({}));
        const title2 = typeof payload.title === "string" ? payload.title.trim() : "";
        const description = typeof payload.description === "string" ? payload.description.trim() : "";
        if (!title2 || !description) {
          return errorResponse(req, env2, 400, "title and description are required");
        }
        const technologies = Array.isArray(payload.technologies) ? payload.technologies.filter((x) => typeof x === "string") : [];
        const insertRow = {
          title: title2,
          description,
        ...
        const { data, error: error3 } = await supabase.from("projects").insert(insertRow).select("*").single();
        if (error3) {
          const code = error3.code;
          if (code === "23505")
            return errorResponse(req, env2, 409, "Project title already exists");
          return errorResponse(req, env2, 500, "Failed to create project", error3);
        }
        return jsonResponse(req, env2, data, 201);
      }
      if (url.pathname === "/api/skills" && req.method === "GET") {
        const { data, error: error3 } = await supabase.from("skills").select("*");
        if (error3)
          return errorResponse(req, env2, 500, "Failed to fetch skills", error3);
        ...
        return jsonResponse(req, env2, Array.from(grouped.values()), 200);
      }
      if (url.pathname === "/api/skills" && req.method === "POST") {
        ...
        if (!category || !name) {
          return errorResponse(req, env2, 400, "category and name are required");
        }
        const level = typeof payload.level === "string" && payload.level.trim() ? payload.level.trim() : "Intermediate";
        const { data, error: error3 } = await supabase.from("skills").insert({ category, name, level }).select("*").single();
        if (error3)
          return errorResponse(req, env2, 500, "Failed to create skill", error3);
        return jsonResponse(req, env2, data, 201);
      }
      if (url.pathname === "/api/contact" && req.method === "POST") {
        ...
        if (!name || !email || !message) {
          return errorResponse(req, env2, 400, "name, email, message are required");
        }
        const { data, error: error3 } = await supabase.from("contacts").insert({ name, email, message }).select("*").single();
        if (error3)
          return errorResponse(req, env2, 500, "Failed to create contact message", error3);
        return jsonResponse(
          req,
          env2,
          {
      ...
      return errorResponse(req, env2, 500, "Internal error", err?.message ?? String(err));
    }
  }
};
export {
